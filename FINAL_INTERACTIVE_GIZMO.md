# 🎯 Финальный Интерактивный Гизмо - Готов к внедрению!

## Статус
- ✅ Plane z-fighting исправлен (translation.y = 0.01)
- ✅ Поля добавлены в класс
- ✅ События добавлены
- ✅ Ray casting функции добавлены
- ⏳ Требуется: Заменить старый renderGizmo + обновить input handlers + обновить renderFrame + обновить performPick

## Быстрое внедрение (копируй-вставляй)

### 1. Удалить старый renderGizmo (строки ~1432-1600)

Найди и удали весь метод `private renderGizmo(gl: WebGL2RenderingContext): void { ... }`

### 2. Вставить новые методы (вместо старого renderGizmo)

```typescript
// ========================================================================
// ИНТЕРАКТИВНЫЙ MOVE GIZMO
// ========================================================================
...
private getRayFromScreen(x: number, y: number): { origin: Vec3; dir: Vec3 } {
	const gl = this.gl!;
	...
	const inv = this.camera.invVpMatrix;
...
private intersectRayPlane(rayO: Vec3, rayD: Vec3, planeP: Vec3, planeN: Vec3, out: Vec3): boolean {
...
private startTranslateDrag(handle: 'tx'|'ty'|'tz', x: number, y: number): void {
	...
	this.intersectRayPlane(ray.origin, ray.dir, center, planeN, this.dragStartHit);
}
...
private updateTranslateDrag(x: number, y: number): void {
	...
	const ray = this.getRayFromScreen(x, y);
	const hit = v3(0,0,0);
	if (!this.intersectRayPlane(ray.origin, ray.dir, center, planeN, hit)) return;
...
### 3. Обновить setupInputHandlers - LMB блок

Найди в `setupInputHandlers()` блок `} else if (e.button === 0) {` и замени на:
...
### 4. Обновить setupInputHandlers - mousemove

В начало `canvas.addEventListener('mousemove', (e) => {` добавь:
...
### 5. Обновить setupInputHandlers - mouseup
...
### 6. Обновить renderFrame - добавить gizmo render

После `renderSelectionOutline(gl);` добавь:
...
### 7. Обновить renderFrame - добавить gizmo picking

После всех render calls, перед `this.renderOverlay();` добавь:
...
### 8. Обновить performPick - сохранять entityId

В конце `performPick`, после вычисления `pickedIndex`, замени на:
...
- ✅ Изменения сохраняются в .vecn
- ✅ Инспектор получает события выбора
