...
export class CodeApplication extends Disposable {
	...
	private registerListeners(): void {
		...
		// File Explorer Refresh Handler
		validatedIpcMain.on('vscode:refreshFileExplorer', () => {
			// Trigger file system watcher refresh for all windows
			const windows = this.windowsMainService?.getWindows();
			if (windows) {
				for (const window of windows) {
					// Send event to trigger file watcher refresh
					window.send('vscode:fileSystemChanged');
				}
			}
			this.logService.info('[Qwen] File explorer refresh triggered');
		});
		...
		// Qwen Chat IPC Handler
		validatedIpcMain.handle('vscode:qwenSendMessage', async (event, query: string, workspacePath?: string) => {
			const { QwenIPCHandler } = await import('./qwenIpc.js');
			const qwenHandler = new QwenIPCHandler(this.logService, workspacePath);
			
			try {
				const result = await qwenHandler.sendMessage(query, workspacePath, (qwenEvent) => {
					// Stream events back to renderer
					event.sender.send('vscode:qwenStreamEvent', qwenEvent);
				});
		...
		// Cargo IPC Handlers
		(async () => {
			const { setupCargoIPC } = await import('./cargoIpc.js');
			setupCargoIPC();
		})();

		// Game Runner IPC Handlers
		(async () => {
			const { setupGameRunnerIPC } = await import('./gameRunner.js');
			setupGameRunnerIPC();
		})();
