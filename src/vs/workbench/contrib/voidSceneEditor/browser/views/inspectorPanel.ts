...
export class InspectorPanel extends Disposable {
        ...
        private showPlaceholder(): void {
                this.sectionsContainer.textContent = '';
        ...
        private renderHistoryTab(): void {
                ...
                const titleDiv = document.createElement('div');
                titleDiv.style.cssText = `margin-bottom: 8px;`;
                titleDiv.textContent = 'История изменений';
                placeholder.appendChild(titleDiv);
                
                const subtitleDiv = document.createElement('div');
                subtitleDiv.style.cssText = `font-size: 11px; color: ${COLORS.textDisabled};`;
                subtitleDiv.textContent = 'Здесь будет отображаться история изменений узла';
                placeholder.appendChild(subtitleDiv);
