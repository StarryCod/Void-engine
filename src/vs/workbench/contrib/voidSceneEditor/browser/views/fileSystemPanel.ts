...
// CSP-safe helper to create SVG from string
function createSVGFromString(svgString: string): SVGSVGElement {
...
export class FileSystemPanel extends Disposable {
        ...
        private showPlaceholder(): void {
                ...
                iconDiv.textContent = icons[this.currentTab];
        ...
        private renderNode(node: FileNode, parent: HTMLElement, depth: number): void {
                ...
                if (node.type === 'folder') {
                        iconEl.appendChild(createSVGFromString(node.expanded ? FILE_ICONS.folderOpen : FILE_ICONS.folder));
                } else {
                        iconEl.appendChild(createSVGFromString(FILE_ICONS[node.extension || ''] || FILE_ICONS.default));
                }
