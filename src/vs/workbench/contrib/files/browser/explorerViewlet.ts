...
import { SceneHierarchyView } from './views/sceneHierarchyView.js';
...
export class ExplorerViewletViewsContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerViews(): void {
		...
		// Scene Hierarchy View - register when workspace is open
		const sceneHierarchyViewDescriptor = this.createSceneHierarchyViewDescriptor();
		if (!viewDescriptors.some(v => v.id === sceneHierarchyViewDescriptor.id)) {
			viewDescriptorsToRegister.push(sceneHierarchyViewDescriptor);
		}
	...
	private createSceneHierarchyViewDescriptor(): IViewDescriptor {
		return {
			id: 'workbench.explorer.sceneHierarchyView',
			name: localize2('sceneHierarchy', "Scene Hierarchy"),
			containerIcon: explorerViewIcon,
			ctorDescriptor: new SyncDescriptor(SceneHierarchyView),
			order: 2,
			canMoveView: true,
			canToggleVisibility: true,
			collapsed: true
